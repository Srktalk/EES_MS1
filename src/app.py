                ...
                dmc.Paper(
                    ...
                    children=[
                        dmc.Title('Pillars of Engagement Drivers', order=2,
                                  style={'font-family': 'IntegralCF-Regular', 'text-align': 'center', 'color': 'grey',
...
def update_metrics(n_clicks):
    ...
    fig_engagement_drivers = px.bar(
        ...
        opacity=0.7,
        color=[color_based_on_threshold(val, low_threshold, mid_threshold, high_threshold) for val in
               engagement_drivers_data.values()],
    ...
    fig_engagement_drivers.update_layout(
        ...
        xaxis=dict(
            ...
            tickmode='array',
            tickvals=list(range(len(engagement_drivers_data))),
            ticktext=[label[:10] + '<br>' + label[10:] if len(label) > 10 else label for label in
                      engagement_drivers_data.keys()],
            tickfont=dict(size=10),
        )
    )
    ...
    fig_emotional_connect = px.bar(
        ...
        labels={'y': 'Engagement Index'},
        text=[f'{min(val, 100):.2f}' for val in emotional_b_index_data.values()],
        # Display the values on top of the bars
        opacity=0.7,
        color=[color_based_on_threshold(min(val, 100), low_threshold, mid_threshold, high_threshold) for val in
               emotional_b_index_data.values()],
        color_discrete_map={'red': 'red', 'gold': 'gold', 'green': 'green'},
        width=10,
    )
    # Customize layout for better appearance
    ...
    fig_work_culture = px.bar(
        ...
        labels={'y': 'Engagement Index'},
        text=[f'{min(val, 100):.2f}' for val in workculture_b_index_data.values()],
        # Display the values on top of the bars
        opacity=0.7,
        color=[color_based_on_threshold(min(val, 100), low_threshold, mid_threshold, high_threshold) for val in
               workculture_b_index_data.values()],
        color_discrete_map={'red': 'red', 'gold': 'gold', 'green': 'green'},
        width=10,
    )
    ...
    fig_scatter = px.scatter(scatter_data, x='Average', y='Correlation', text='Parameter', size='Average',
                             color='Parameter',
